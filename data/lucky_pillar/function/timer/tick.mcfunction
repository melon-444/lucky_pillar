...
#死斗开启后死亡自动掉落物品
execute if data storage lucky_pillar:io {game_state:"running"} if score $death_match lp_cnt matches 0 run scoreboard players set @a death_cnt 0
...
#死斗阶段淘汰广播（包含赢家判断）
execute if data storage lucky_pillar:io {game_state:"running"} if score $death_match lp_cnt matches 1 run function lucky_pillar:services/predict_population